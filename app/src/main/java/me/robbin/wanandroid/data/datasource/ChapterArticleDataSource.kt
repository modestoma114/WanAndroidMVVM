 ...
 */
class ChapterArticleDataSource(private val type: Int) : PagingSource<Int, ArticleBean>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ArticleBean> {
        return try {
            ...
            val response =
                if (type == 1)
                    ApiService.getApi().getQuestionArticleList(page)
                else
                    ApiService.getApi().getUserArticleList(page)
            LoadResult.Page(
